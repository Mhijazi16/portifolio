  ...
  return (
    <section id="work" className="py-24 bg-black/40">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          {workExperiences.map((work, index) => {
            const translated = t.experience.items[work.id];
            return (
              <div
                key={work.id}
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-100 transition-opacity duration-300 flex flex-col justify-end p-8">
                  <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2">{translated?.title || work.title}</h3>
                    <p className="text-neutral-300 text-sm line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
                      {translated?.description || work.description}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <Modal
        ...
        title={selectedWork ? (t.experience.items[selectedWork.id]?.title || selectedWork.title) : ''}
      >
        <div className="space-y-6">
          ...
          />
          <div>
            <h4 className="text-lg font-semibold text-white mb-2">{t.modal.roleResponsibilities}</h4>
            {selectedWork?.duration && (
              <p className="text-sm text-purple-400 mb-2 font-medium">{selectedWork.duration}</p>
            )}
            <p className="text-neutral-300 leading-relaxed mb-4">
              {selectedWork ? (t.experience.items[selectedWork.id]?.description || selectedWork.description) : ''}
            </p>
          </div>
          <div>
            <h4 className="text-lg font-semibold text-white mb-2">{t.modal.techStack}</h4>
